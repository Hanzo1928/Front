...
export type Options = { convertToUpperCase: boolean; unknownChar?: SegmentName[] | 'exception' };

const DEFAULT_VIEW_OPTIONS: Options = { convertToUpperCase: true};

export function marquee(
  input: string,
  displayAmount: number = input.length,
  options: Options = DEFAULT_VIEW_OPTIONS
): SegmentName[][][] {
  const frames: SegmentName[][][] = [];
